...
\begin{figure}
	...
	\caption{
        Toy example of shear-dependent detection in the presence of a PSF.  In
        the top row of plots, for which there was no PSF convolution, and it
        can be seen that the inner contours do not overlap before or after
        shearing. This is a general property of the shear transformation.
        However, when the image is convolved by a PSF, which is independent of
        the transformation, the inner contours overlap only after shearing.  A
        detection algorithm identified overlapping contours as one object, this
        would manifest as a shear-dependent detection bias.  \label{fig:toy}
    }
