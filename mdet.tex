...
for simultaneously. We call this technique \mdet.  An rough outline of the method
is as follows (we give more detail in \S \ref{sec:mdet}):
\begin{enumerate}
    ...
    \item Calculate ensemble shear statistics using measurements from the
        unsheared images.  We assume our basic shear estimator responds
        approximately linearly to a shear.

    \item Use measurements from sheared images to calculate how the
        measurements responds to a shear.
...
different catalogs, whereas in \cite{SheldonMcal2017} all measurements were
performed using a static detection catalog.
...
\section{\Mdet} \label{sec:mdet}

In \cite{SheldonMcal2017} we wrote the mean ellipticity
as
\begin{equation}
...
calculating a shear response and selection response separately.
    ...
    Use measurements from sheared images to calculate an ensemble shear
    ...
    Correct the ensemble shear statistics using the response: $\langle \boldsymbol\gamma \rangle \approx \langle \boldsymbol{R}\rangle^{-1}\langle\boldsymbol{e}\rangle$.
