...
In order to apply \mdet\ to a real, multiband, multiepoch survey like the
...
but create the PSF model by stacking some number of variable PSF models. In order
...
for details. With only stacking one of the PSF models in Appendix~\ref{app:pspsf},
we find a multiplicative bias of $XYZ$. However, once we stack five of these models,
which is a minimum amount of coadding expected for both the DES and LSST, we find
a multiplicative bias of only $XYZ$. Thus we expect that in a realistic survey
