...
\begin{abstract}
    ...
    current surveys and increases with increasing galaxy number density.  This
    bias is due to shear-dependent object detection rather than object blending
    itself; if object detection is shear independent, no deblending of images
    is needed, in principle.  We demonstrate that shear-dependent object
    detection biases are accurately removed when including object detection in
    the \mcal\ process, a technique we call \mdet. We furthermore show that the
    primary technical challenge, deconvolution with a spatially varying
    point-spread-function, does not result in a significant bias for typical
    imaging surveys.  Finally, we discuss the physical limits of \mdet's
    accuracy.  The key assumption behind \mdet\ is that the entire region of
    sky under consideration is sheared, including the space between objects, as
    opposed to only the shapes of the objects.  In the real universe, lensing
    over cosmological distances produces a combination of these scenarios.  We
    test a worst case scenario for \mdet, in which only the object shapes are
    sheared, and find the bias should be at most a few tenths of a percent for
    future surveys.  We discuss the technical challenges that must be met
    in order to implement \mdet\ in real surveys.
