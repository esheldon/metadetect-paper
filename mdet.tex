...
prep.).  We got similar results using a simple local peak finder for
...
% s2n: 10
%     # of sims: 169018
%     m [1e-3]: -4.140912 +/- 0.926520
%     c [1e-4]: -0.140211 +/- 0.290522
% s2n: 15
%     # of sims: 169018
%     m [1e-3]: -5.266391 +/- 0.781841
%     c [1e-4]: -0.263428 +/- 0.311595
% s2n: 20
%     # of sims: 169018
%     m [1e-3]: -5.371570 +/- 0.712035
%     c [1e-4]: -0.312051 +/- 0.338221
...
In order to obtain an upper bound on this effect, we made a simple modification
to our simulations. When building them, rather than shearing the full scene to
impart the true shear to the image, we sheared each object individually and
then added it to the image, without any change in the object position. This
modification leaves the space between objects unsheared, which is maximally
different from what happens during the \mcal\ image shearing process. Results
for these simulations are in the bottom rows of Table~\ref{tab:shearmeas}. We
found small residual biases in this case, of order $\sim-0.3\%$.

We consider this a rough upper bound on the effect, more study is needed to
address the bias in specific imaging surveys.  We expect larger biases for
deeper and more dense surveys, and for surveys with larger PSFs.  Predicting
the expected level of this bias for future experiments such as such as DES and
LSST is essential, but beyond the scope of this work.
...
\mcal\ process, even for an LSST-like survey. This technique, which we call
...
implementing \mdet\ on real data. These challenges arise from the fact that
\mdet\ applies artificial shears to relatively large images ($\sim1-2$
arcminutes on a side) and that \mdet\ catalogs cannot be matched to other
object catalogs. The size of the image used for \mdet\ is dictated by the
need to accurately capture the effects of shearing the image on the object
detection process. If we make this image significantly smaller, we may not
accurately capture ambiguities in the detection of objects, especially for
blends where one object is on the edge of the image. Making this image
significantly larger could complicate other parts of the analysis related to PSF
deconvolution and wold coordinate system (WCS) handling, as detailed below.
Finally, \mdet\ catalogs cannot be matched to other catalogs because this
process would reintroduce object detection biases.

Over these large scales, the assumption that the PSF and wold coordinate system
(WCS) transformations are approximately constant will fail, complicating the
...
However, in order to reconstruct accurate PSF models for the coadd, the
input images can have no edges within the coadd region. This fact will result
the loss of some precision \citep{ArmstrongCoadd}. Finally, we should be able
to handle non-constant WCS transformations by coadding the images into a nearly
constant WCS.

Unfortunately, additional issue arise from masking. Large regions of images can
have non-trivial masking patterns due to stellar diffraction spikes, cosmic
rays, etc. In the implementation presented in this work, we use fast Fourier
transforms (FFTs) to handle convolutions. The FFT does not permit missing data,
so the masked regions must be interpolated in some way.  Care must be taken that
this interpolation does not introduce a spurious shear signal.  Compensating
masks, rotated at right angles to the real masks, must be used in addition to
the real mask in order to restore symmetry to the image \citep{SheldonMcal2017}.
Interpolation correlates the noise in the image, as does the coadding process
itself, so the noise field used for correcting correlated noise effects must
also be propagated through the same coadding and interpolation
...
catalogs to each other or any external catalog. They also must apply the same
