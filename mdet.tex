...
\author[Sheldon et~al.]{Erin Sheldon$^1$, Matthew R. Becker$^2$, Eric M. Huff$^3$,
Niall MacCrann$^{4,5}$, Michael Jarvis$^6$
...
regime, and the mapping preserves surface brightness, detection need not be
shear dependent \citep{SchneiderBook92}.  For example, a detection algorithm
that simply identifies connected regions above some threshold as a single
object will not in principle be shear dependent. However, if the image is
smeared by a point-spread-function (PSF), due to the atmosphere and detector,
objects overlap differently after shearing, because the PSF convolution occurs
after the shear mapping.  This effect is demonstrated in figure \ref{fig:toy}.
...
\begin{figure*}
    ...
    shear.  In panel b) the objects are shared by a shear $(0.0, 0.1)$ {\em
    after} the PSF convolution. In this case the inner contours for the two
    objects overlap before and after application of the shear.  This is a
    general property of the shear transformation in the weak regime.   In panel
    ...
    some threshold as a single object would manifest a shear-dependent
    detection bias.  \label{fig:toy} }

\end{figure*}

Any shear-dependent measurement effects can in principle be taken into account
with the \mcal\ formalism.  The implementations presented in
...
extension of the algorithm, which we will address in turn.
