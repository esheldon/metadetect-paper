...
challenges.  These images can have non-trivial world coordinate system (WCS)
variation, which may have significant spatial variation, which complicates
the application of an artificial shear. The easiest way to deal with this
would be to produce a coadd with nearly constant WCS.  Larger regions of images
can have non-trivial masking patterns due to stellar diffraction spikes, cosmic
...
so the masked regions must be interpolated in some way.  In order to
reconstruct accurate PSF models for the coadd, the input images can have no
edges within the coadd region.  Images with edges must be left out of
the coadd, resulting in some loss of precision \citep{ArmstrongCoadd}.  Finally, the
five separate \mdet\ catalogs must each be incorporated into the full set of
...
constraints. The degree to which these technical challenges can be overcome
will ultimately determine the accuracy of \mdet\ when used to analyze imaging
survey data.
