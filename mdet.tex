...
\begin{abstract}
    ...
    shear is then calculated, measurements which now include detection itself,
    and this response is used to calibrate the final shear estimate.  We show
    that the primary technical challenge for \mdet, deconvolution with a
    ...
    produces a combination of these scenarios.  We test a worst case scenario
    for \mdet, in which only the object images are sheared, and find the bias
    is at most a few tenths of a percent for future surveys.  We discuss the
    technical challenges that must be met in order to implement \mdet\ in real
    surveys.
