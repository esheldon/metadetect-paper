...
First, we must run \mdet\ over relatively large images ($\sim1-2$ arcminutes
on a side) in order to accurately capture ambiguities in the detection of
objects.  Detection is less efficient in smaller images, because the
larger perimeter to area ratio results in a higher fraction of objects, including
blends, near the edge.
Over these large scales, the assumption that the PSF and world
...
result in the loss of some precision \citep{ArmstrongCoadd}. Finally,
we should be able to handle non-constant WCS transformations by coadding the
images into a nearly constant WCS, but this procedure remains to be tested.

Second, additional issues arise from masking. Large regions of images can
