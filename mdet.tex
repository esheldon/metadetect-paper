...
implementing \mdet\ on real data.  With \mdet\ we must apply artificial shears
to relatively large images ($\sim1-2$ arcminutes on a side).  We have shown
that PSF variation is unlikely to be a problem, but there are additional
challenges.

The images input to the coadd can have significant spatial variations in
their world coordinate system (WCS) transformations,  which complicates the
application of an artificial shear. The easiest way to deal with this would be
to produce a coadd with nearly constant WCS.

Large regions of images can have non-trivial masking patterns due to stellar
diffraction spikes, cosmic rays, etc. In the implementation presented in this
work, we use fast Fourier transforms (FFTs) to handle convolutions. The FFT
does not permit missing data, so the masked regions must be interpolated in
some way.  Care must be taken that this interpolation does not introduce a
spurious shear signal.  Compensating masks, rotated at right angles to the real
masks, must be used in addition to the real mask, as discussed in
\cite{SheldonMcal2017}.  Interpolation correlates the noise in the image, as
does the coadding process itself, so the noise field used for correcting
correlated noise effects must also be propagated through the same coadding
and interpolation \citep{SheldonMcal2017,ArmstrongCoadd}.

In order to reconstruct accurate PSF models for the coadd, the input
images can have no edges within the coadd region.  Images with edges must be
left out of the coadd, resulting in some loss of precision
\citep{ArmstrongCoadd}.

Finally, the five separate \mdet\ catalogs must each
...
in order to be used for cosmological constraints. The degree to which these
technical challenges can be overcome will ultimately determine the accuracy of
